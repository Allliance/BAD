...
def get_ood_loader(in_dataset=None, out_dataset=None, sample=True, sample_num=2000, in_label=1,
                   ...
                   in_transform=None, out_transform=None, custom_ood_dataset=None, custom_in_dataset=None, **kwargs):
    ...
    if custom_ood_dataset is None:
        if out_dataset == 'rot':
            ...
            transform = transforms.Compose([lambda x: rotate(x, 90), transform])
            if isinstance(out_dataset, Subset):
                out_dataset.dataset.transform = transform
            else:
                out_dataset.transform = transform
